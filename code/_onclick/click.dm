...
/mob/proc/ClickOn(var/atom/A, var/params)
	...
	if((!isturf(A) && A == loc) || (sdepth != -1 && sdepth <= 1))
		...
		if(W)
			var/resolved = (SEND_SIGNAL_LEGACY(W, COMSIG_IATTACK, A, src, params)) || (SEND_SIGNAL_LEGACY(A, COMSIG_ATTACKBY, W, src, params)) || W.resolve_attackby(A, src, params)
			if(!resolved && A && W)
...
/atom/proc/CtrlClick(var/mob/user)
	SEND_SIGNAL_LEGACY(src, COMSIG_CLICK_CTRL, user)
	return
...
/atom/proc/AltClick(var/mob/user)
	SEND_SIGNAL_LEGACY(src, COMSIG_CLICK_ALT, user)
