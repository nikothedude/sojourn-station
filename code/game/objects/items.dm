...
/obj/item/Initialize()
	...
	for (var/upgrade_typepath in initialized_upgrades)
		...
		if (!(SEND_SIGNAL_LEGACY(upgrade, COMSIG_IATTACK, src, null)))
			QDEL_NULL(upgrade)
...
/obj/item/proc/pickup(mob/target)
	...
	SEND_SIGNAL_LEGACY(src, COMSIG_ITEM_PICKED, src, target)
...
/obj/item/proc/on_exit_storage(obj/item/storage/the_storage)
	SEND_SIGNAL_LEGACY(the_storage, COMSIG_STORAGE_TAKEN, src, the_storage)
	return

// called when this item is added into a storage item, which is passed on as S. The loc variable is already set to the storage item.
/obj/item/proc/on_enter_storage(obj/item/storage/the_storage)
	SEND_SIGNAL_LEGACY(the_storage, COMSIG_STORAGE_INSERTED, src, the_storage)
	return
