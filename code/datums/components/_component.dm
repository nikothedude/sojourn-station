...
/datum/component/Destroy(force=FALSE, silent=FALSE)
	if(!force && parent)
		_RemoveFromParent()
	if(!silent)
		SEND_SIGNAL_LEGACY(parent, COMSIG_COMPONENT_REMOVING, src)
	parent = null
	return ..()
...
/datum/proc/AddComponent(new_type, ...)
	...
	if(!old_comp && !QDELETED(new_comp)) // Nothing related to duplicate components happened and the new component is healthy
		SEND_SIGNAL_LEGACY(src, COMSIG_COMPONENT_ADDED, new_comp)
		return new_comp
	return old_comp
...
/datum/component/proc/RemoveComponent()
	...
	_RemoveFromParent()
	parent = null
	SEND_SIGNAL_LEGACY(old_parent, COMSIG_COMPONENT_REMOVING, src)
